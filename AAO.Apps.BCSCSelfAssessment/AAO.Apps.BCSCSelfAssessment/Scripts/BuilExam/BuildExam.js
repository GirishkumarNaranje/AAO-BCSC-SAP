(function () {
    ...
    // Enhanced email validation function with better error handling and edge case support
    function validateMultipleEmailsCommaSeparated(value) {
        ...
        // Remove any trailing/leading whitespace and normalize commas
        var cleanedValue = value.trim().replace(/,\s*,/g, ',').replace(/^,+|,+$/g, '');

        if (cleanedValue === '') {
            //console.log('Email validation failed: empty after cleaning');
            return false;
        }

        var emails = cleanedValue.split(",");
        var validEmails = 0;

        for (var i = 0; i < emails.length; i++) {
            var email = emails[i].trim();
            if (email === '') {
                continue; // Skip empty entries (like trailing commas)
            }
            //console.log('Validating individual email:', email);
    ...
    // Enhanced single email validation function with better regex
    function validateEmail(field) {
        if (!field || field.trim() === '') {
            return false;
        }

        var email = field.trim();
    ...
    function validateMultipleEmails() {
        var emailValue = $('#emailInput_text').val();

        if (!emailValue || emailValue.trim() === '') {
            $('#emailInput_text').focus();
            $('.EmailIdSpan').text('Please enter at least one email address.');
            return false;
        }

        if (!validateMultipleEmailsCommaSeparated(emailValue)) {
            $('#emailInput_text').focus();
            $('.EmailIdSpan').text('Please enter valid email addresses separated by commas.');
            return false;
        }
    ...
    // Function to validate multiple emails for shared exam
    function validateSharedExamEmails() {
        var emailValue = $('#emailInput_text').val();
    ...
    function wireSubmitToEnableDisabledFields() {
        // This function is now handled in the Create button click handler
        // to avoid conflicts with form submission
    }
    ...
    $("#ExamType").on('click', function () {
        ...
        if (typeof window.clearAllEmails === 'function') {
            ...
            $('#emailInput_text').val(''); // Clear the email field
        }
        $('.EmailIdSpan').text(''); // Clear email error messages
    });
    ...
    $("#Create").click(function (e) {
        //console.log('Create button clicked - starting validation');

        // Prevent default button behavior first
        e.preventDefault();
        e.stopPropagation();
        ...
        if (isSharedExam) {
            // Enhanced email validation for shared exams with better debugging
            var emailValue = $('#emailInput_text').val();
            //console.log('Create button - Email validation. Email value:', emailValue);

            if (!emailValue || emailValue.trim() === '') {
                //console.log('Email validation failed in Create button: empty');
                $('#emailInput_text').focus();
                $('.EmailIdSpan').text('Please enter at least one email address.');
                flag = false;
                return false;
            ...
            if (!validateMultipleEmailsCommaSeparated(emailValue.trim())) {
                //console.log('Email validation failed in Create button: invalid format');
                $('#emailInput_text').focus();
                $('.EmailIdSpan').text('Please enter valid email addresses separated by commas. Example: user1@example.com, user2@example.com');
                flag = false;
                return false;
            }
            ...
            $('.EmailIdSpan').text('');
            //console.log('Email validation passed in Create button');

            // Date validation for shared exams
            /*if ($("#DateRangeEnabled").prop("checked")) {
                ...
                console.log("todayDateStr (local YYYY-MM-DD):", todayDateStr);
                ...
                // Check if start date is not in the past (local date comparison)
            ...
            if ($("#DateRangeEnabled").prop("checked")) {
                ...
                //console.log("startDateStr - original:", startDateStr); console.log("endDateStr - original:", endDateStr); console.log("todayDate_New - original", todayDate_New);
                ...
                function toUTCDate(dateStr) {
                    //console.log(" -- UTC conversion -- "); console.log("Input dateStr:", dateStr);
                    //
                    ...
                    // Create local date:  2026-01-14
                    var date1 = new Date(dateStr + 'T00:00:00'); // Treats as local time

                    // Adjust for timezone offset
                    var timezoneOffset1 = date1.getTimezoneOffset() * 60 * 1000;
                    var utcDate1 = new Date(date1.getTime() + timezoneOffset1);

                    // Set time to 00:00:00.000 UTC (midnight)
                    var aaa = utcDate1.setUTCHours(0, 0, 0, 0);

                    //console.log("Input:", dateStr);
                    //console.log("User timezone offset (min):", date1.getTimezoneOffset());
                    //console.log("UTC Date:", utcDate1.toISOString()); // Returns: 2026-01-13T00:00:00.000Z (for IST user)
                    //console.log("utcDate1.getTime():", utcDate1.getTime());
                    //console.log("aaa:", aaa);
                ...
                var startDateUTC1 = toUTCDate(startDateStr);
                var endDateUTC1 = toUTCDate(endDateStr);
                ...
                if (endDateUTC1 < startDateUTC1) {
                    //console.log("endDateStr < startDateStr :", endDateStr < startDateStr);
                    $("#EndDate").focus();
                    $(".EndDateSpan").text("End date must be after start date.");
                    flag = false;
                    return false;
                ...
                if (startDateUTC1 < todayDateUTC) {
                    //console.log("startDateStr < todayDate :", startDateStr < todayDate);
                    $("#StartDate").focus();
                    $(".StartDateSpan").text("Start date cannot be in the past.");
                    flag = false;
                    return false;
        ...
        //console.log('All validations completed. Flag status:', flag);

        if (flag == true) {
            //console.log('All validations passed - preparing form submission');
            // Clear any previous error messages
            $('.EmailIdSpan').text('');

            // Enable any disabled fields before submission
            $('#ShowAnsAfterExamCompletion, #StartDate, #EndDate').prop('disabled', false);

            // Set the exam create date and UTC dates for shared exams
            setExamCreateDate();
            if (isSharedExam) {
                setExamStart_EndDate();
            }

            // Submit the form using a more reliable method
            //console.log('Submitting form to controller...');

            // Remove any existing event handlers to avoid conflicts
            $('#createExam').off('submit');
    ...
    $("#Cancel").on('click', function () {
        ...
        // Reset form
        document.getElementById('createExam').reset();
        ...
        $('.error-text').text('');
        $('input, textarea').removeClass('input-validation-error').css('border-color', '');
    });
...
function searchAAOSharedExam() {
    ...
    if (searchitem.trim() !== "") {
        ...
        //console.log('StartDate field value:', $('#StartDate').val());
        //console.log('EndDate field value:', $('#EndDate').val());
        //console.log('StartDateUTC field value:', $('#StartDateUTC').val());
        //console.log('EndDateUTC field value:', $('#EndDateUTC').val());
        //console.log('Collected formData:', formData);
        //console.log('formData.startDateUTC:', formData.startDateUTC);
        //console.log('formData.endDateUTC:', formData.endDateUTC);
...
function collectSharedExamFormData(searchTerm) {
    ...
    try {
        ...
        // ADDED: Capture the UTC date values
        /*formData.StartDateUTC = $('input[name="StartDateUTC"]').length > 0 ? ($('input[name="StartDateUTC"]').val() || '') : '';
        formData.EndDateUTC = $('input[name="EndDateUTC"]').length > 0 ? ($('input[name="EndDateUTC"]').val() || '') : '';
*/
        // ADDED: Capture the UTC date values - NOTE: Case sensitive, must match HTML name attribute exactly
        formData.startDateUTC = $('input[name="StartDateUTC"]').length > 0 ? ($('input[name="StartDateUTC"]').val() || '') : '';
        formData.endDateUTC = $('input[name="EndDateUTC"]').length > 0 ? ($('input[name="EndDateUTC"]').val() || '') : '';

        // Handle multiple emails
        var emailValue = '';
        var emailInputElement = $('textarea[name="EmailId"], input[name="EmailId"]');
        if (emailInputElement.length > 0) {
            emailValue = emailInputElement.val() || '';
        } else {
            emailValue = $('#emailInput_text').length > 0 ? ($('#emailInput_text').val() || '') : '';
        }
...
$(document).ready(function () {
    try {
        // Simple date range initialization
        // Check if date range is enabled and enable fields accordingly
        var dateRangeEnabled = $('#DateRangeEnabled').is(':checked');

        //console.log('Date restoration - DateRangeEnabled:', dateRangeEnabled);

        if (dateRangeEnabled) {
            // Enable date fields if DateRangeEnabled is checked
            $('#StartDate, #EndDate').prop('disabled', false);
            //console.log('Date fields enabled on page load');

            // Apply visual styling
            if (typeof enableDateRangeFeatures === 'function') {
                enableDateRangeFeatures();
            }
            return; // Exit early if we restored the state
        }

        // Original date restoration logic for other scenarios
        var fd = null;
        ...
        if (isTrue) {
            ...
            if (fd.StartDate && fd.StartDate.toString().trim() !== '') {
                var startDateFormatted = formatDateForInput(fd.StartDate);
                $('#StartDate').val(startDateFormatted);
            }
            if (fd.EndDate && fd.EndDate.toString().trim() !== '') {
                var endDateFormatted = formatDateForInput(fd.EndDate);
...
/*$(document).ready(function () {
    ...
    $('#emailInput_text').on('blur', function () {
        var emailValue = $(this).val();
        var $this = $(this);
        var $errorSpan = $('.EmailIdSpan');
        
        if (emailValue && emailValue.trim() !== '') {
            if (!validateMultipleEmailsCommaSeparated(emailValue.trim())) {
                $errorSpan.text('Please enter valid email addresses separated by commas. Example: user1@example.com, user2@example.com');
                $this.addClass('input-validation-error').css('border-color', '#dc3545');
            } else {
                $errorSpan.text('');
                $this.removeClass('input-validation-error').css('border-color', '#28a745');
...
/*var allowSharedExamSubmit = false;
$('#createExam').off('submit.sharedExamCheck').on('submit.sharedExamCheck', function (e) {
    var examType = $('#ExamType').val();
    console.log("examType :", examType);
    console.log("examType :", examType === '4', examType == 4);
    if (examType === '4' && !allowSharedExamSubmit) {
        e.preventDefault();
        setExamCreateDate();
        setExamStart_EndDate();
        var emails = ($('#emailInput_text').val() || '').split(',').map(function (x) { return x.trim(); }).filter(Boolean).join(',');
        if (!emails) {
            // No emails entered, let form submit and let server handle validation
            allowSharedExamSubmit = true;
            $('#createExam').submit();
            return;
        }
        $.ajax({
            url: '/ExamManager/CheckSubscribedEmails',
            type: 'POST',
            data: { emails: emails },
            success: function (res) {
                if (res && res.duplicates && res.duplicates.length > 0) {
                    // Show modal with unsubscribed emails
                    var $list = $('#emailNotSubscribedList').empty();
                    res.duplicates.forEach(function (em) {
                        $('<li/>').text(em).appendTo($list);
                    });
                    $('#EmailNotSubscribedModal').modal('show');
                    // On proceed, allow submit
                    $('#reshare-dup-proceed').off('click').on('click', function () {
                        $('#EmailNotSubscribedModal').modal('hide');
                        allowSharedExamSubmit = true;
                        $('#createExam').submit();
                    });
                    // On cancel, do nothing (modal will close)
                    $('#reshare-dup-cancel').off('click').on('click', function () {
                        $('#EmailNotSubscribedModal').modal('hide');
                    });
                } else {
                    // All emails subscribed, proceed
                    allowSharedExamSubmit = true;
                    $('#createExam').submit();
                }
            },
            error: function () {
                // On error, allow submit (fail open)
                allowSharedExamSubmit = true;
                $('#createExam').submit();
            }
        });
...
// Keep this simpler version at the end of BuildExam.js
$(document).ready(function () {
    $('#emailInput_text').on('input keyup', function () {
        $(this).removeClass('input-validation-error').css('border-color', '');
        $('.EmailIdSpan').text('');
    ...
    // Simple form submit handler - let controller handle email checking
    $('#createExam').on('submit', function (e) {
        //console.log('Form submit event triggered');
        ...
        //console.log('Form submission proceeding to server');
        // Let the form submit naturally - controller will handle email checking
    });
});
